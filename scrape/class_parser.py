        ...
        for tdTag in trTag.find_all('td'):                                                                              # have the "valign='top'" attribute to it, so to scrape from
            if len(tdTag.text) > 0:
                ...
                if (len(piece_of_info) != 5) and (counter == 0):                                                        # Check to see if this is a class code or course title
                    ...
                    current_course = crs.Course(name.strip())                                                           # Create a new course i.e. after adding ICS 6B now we are working on 31
...
def remove_prerequisite(name_of_course):
    """
    While web-scraping the name of the course, there is a trailing word "(Prerequisite)", this function simply removes that word so that it can be easily read later on.
